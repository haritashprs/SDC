<?php 
	...
	Class Data extends Dbh {
		...
		public function styleSheet($sheet, $row, $spreadsheet) {
			...
			$sheet->getStyle('A1:Q2')->applyFromArray($titleStyleArray);
			$sheet->getStyle('A3:Q3')->applyFromArray($headingStyleArray);
			$sheet->getStyle('A4:Q'.($row-1))->applyFromArray($dataStyleArray);
		...
		public function getData($sheet, $spreadsheet) {
			...
			if ($stmt->execute(['S','Clueminati', 'Offline'])) {
				$row = 4;
				while ($data = $stmt->fetch()) {

					//make Table Headers
					$sheet->setCellValue('A1', 'Clueminati Complete Registration List');
					...
					$sheet->setCellValue('O3', 'PARTIAL AMOUNT');
					$sheet->setCellValue('P3', 'BALANCE AMOUNT');
					$sheet->setCellValue('Q3', 'DATE');
					$sheet->setCellValue('R3', 'TIME');
					...
					$sheet->setCellValue('O'.$row, $data['partial_amount']);
					$sheet->setCellValue('P'.$row, $data['bal_amount']);
					$sheet->setCellValue('Q'.$row, $data['date']);
	...
	header('Content-Disposition: attachment;filename="Clueminati.xlsx"');
