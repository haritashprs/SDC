<?php 
	...
	Class Data extends Dbh {
		...
		public function styleSheet($sheet, $row, $spreadsheet) {
			...
			$sheet->getStyle('A1:W2')->applyFromArray($titleStyleArray);
			$sheet->getStyle('A3:W3')->applyFromArray($headingStyleArray);
			$sheet->getStyle('A4:W'.($row-1))->applyFromArray($dataStyleArray);
		...
		public function getData($sheet, $spreadsheet) {
			...
			if ($stmt->execute(['S','Projectmania', 'Offline'])) {
				$row = 4;
				while ($data = $stmt->fetch()) {

					//make Table Headers
					$sheet->setCellValue('A1', 'Projectmania Complete Registration List');
					...
					$sheet->setCellValue('Q3', 'NAME 5');
					$sheet->setCellValue('R3', 'EMAIL 5');
					$sheet->setCellValue('S3', 'DIVISION');
					$sheet->setCellValue('T3', 'PARTIAL AMOUNT');
					$sheet->setCellValue('U3', 'BALANCE AMOUNT');
					$sheet->setCellValue('V3', 'DATE');
					$sheet->setCellValue('W3', 'TIME');
					...
					$sheet->setCellValue('Q'.$row, $data['name5']);
					$sheet->setCellValue('R'.$row, $data['email5']);
					$sheet->setCellValue('S'.$row, $data['division']);
					$sheet->setCellValue('T'.$row, $data['partial_amount']);
					$sheet->setCellValue('U'.$row, $data['bal_amount']);
					$sheet->setCellValue('V'.$row, $data['date']);
					$sheet->setCellValue('W'.$row, $data['time']);
	...
	header('Content-Disposition: attachment;filename="Projectmania.xlsx"');
