<?php 
	...
	Class Data extends Dbh {
		...
		public function getData($sheet, $spreadsheet) {
			...
			if ($stmt->execute(['S','Robosoccer', 'Online'])) {
				$row = 4;
				while ($data = $stmt->fetch()) {

					//make Table Headers
					$sheet->setCellValue('A1', 'Robosoccer Complete Registration List');
	...
	header('Content-Disposition: attachment;filename="Robosoccer.xlsx"');
