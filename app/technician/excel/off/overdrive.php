<?php 
	...
	Class Data extends Dbh {
		...
		public function getData($sheet, $spreadsheet) {
			...
			if ($stmt->execute(['S','Overdrive', 'Offline'])) {
				$row = 4;
				while ($data = $stmt->fetch()) {

					//make Table Headers
					$sheet->setCellValue('A1', 'Overdrive Offline Registration List');
	...
	header('Content-Disposition: attachment;filename="Overdrive.xlsx"');
