<?php 
  ...
  Class Dashboard extends Dbh {
    ...
    public function eventDetails($event) {
      $sql= "SELECT event, COUNT(event) AS 'total_reg', SUM(amount) AS 'total_amt', SUM(bal_amount) AS 'bal_amt', SUM(partial_amount) AS 'paid_amt', SUM(no_members) AS 'members' FROM registration WHERE status=? AND event=?";
      $stmt = $this->connect()->prepare($sql);
      if ($stmt->execute(['S', $event])) {
        $row = $stmt->fetch();
        if ($row['event'] != NULL) { ?>
          <tr>
